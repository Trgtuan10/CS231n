...
def affine_forward(x, w, b):
    ...
    x_flatten = np.reshape(x, (x.shape[0], np.prod(x.shape[1:])))
...
def relu_backward(dout, cache):
    ...
    Input:
    - dout: Upstream derivatives, ocache f any shape
...
def batchnorm_backward_alt(dout, cache):
    ...
    dgamma = np.sum(x_hat * dout, axis = 0)
    dbeta = np.sum(dout , axis = 0)                                         # full derivative w.t.r. x
...
def layernorm_forward(x, gamma, beta, ln_param):
    ...
    cache = (x, x_norm, gamma, mean_layer, var_layer, eps)
...
def layernorm_backward(dout, cache):
    ...
    x, x_norm, gamma, mean_layer, var_layer, eps = cache
    N, D = x.shape
    ...
    # Partial derivatives
    dvar = np.sum(dx_norm * (x - mean_layer) * -0.5 * np.power(var_layer + eps, -1.5), axis=1, keepdims=True)
    dmean = np.sum(dx_norm * -1 / np.sqrt(var_layer + eps), axis=1, keepdims=True) + dvar * np.mean(-2 * (x - mean_layer), axis=1, keepdims=True)
    dx = (dx_norm / np.sqrt(var_layer + eps)) + (dvar * 2 * (x - mean_layer) / D) + (dmean / D)
...
def dropout_forward(x, dropout_param):
    ...
    Outputs:
    - out: Array of the same sh ape as x.
        ...
        for k in range(F):
            for i in range(int(H_out)):
                for j in range(int(W_out)):
                    ...
                    out[n, k, i, j] = np.sum(x_pad_mask * w[k, :, :, :])
...
def spatial_batchnorm_forward(x, gamma, beta, bn_param):
    ...
    x = np.moveaxis(x, 1, -1).reshape(-1, C)                 # swap axes 
    out, cache = batchnorm_forward(x, gamma, beta, bn_param) # batchnorm
...
def svm_loss(x, y):
    ...
    correct_class_sc = x[range(num_train), list(y)].reshape(-1,1)
    # print(correct_class_sc.shape)
    margins = np.maximum(0, 1 + x - correct_class_sc)
...
def softmax_loss(x, y):
    ...
    x_exp = np.exp(x)
    scores = x_exp / np.sum(x_exp, axis=1).reshape(-1,1)
